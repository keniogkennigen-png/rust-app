...
/// Simplified user data for JSON responses (No Mutexes here!)
...
/// Internal User struct (Removed Serialize to fix Mutex errors)
#[derive(Debug, Clone)]
pub struct User {
    pub id: Uuid,
    pub username: String,
...
async fn create_session(user: &User, app_state: Arc<AppState>) -> AuthResponse {
    ...
    app_state.user_sessions.lock().await.insert(new_session_key.clone(), UserSession {
        user_id: user.id,
